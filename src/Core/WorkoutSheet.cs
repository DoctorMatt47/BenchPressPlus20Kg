...
{
    public static List<List<SetRelative>> GetRelativePlan()
    {
        return new List<List<IEnumerable<SetRelative>>>
            {
        ...
        bool isFailureTest = false)
    {
        for (var i = 0; i < count; i++)
        {
            ...
            {
                (false, false) => new SetRelative {Percent = percent, Reps = reps},
                (true, false) => new SetRelative {Percent = percent, IsNegative = true},
                (false, true) => new SetRelative {Percent = percent, IsFailureTest = true},
